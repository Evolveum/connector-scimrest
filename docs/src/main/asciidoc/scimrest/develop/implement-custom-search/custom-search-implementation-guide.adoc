...
If the list lives at a fixed location, a path instead of a closure keeps the
configuration declarative: `objectExtractor('$._embedded.elements')` (basic
JSONPath, single quotes because Groovy double-quoted strings interpolate `$`)
or an explicit `{ type, value }` path — JSON Pointer
(`/_embedded/elements`) or JSONPath, as in the attribute `path` keys. A
resolved array yields its elements, a resolved object yields the single
object, and a missing path yields no objects.
...
configuration options, available objects and methods, and examples, see xref:index.adoc[].